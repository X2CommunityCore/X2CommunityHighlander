...
private function Vector GetSpawnLocation()
{
	...
	foreach `BATTLE.AllActors(class'XComGroupSpawn', Spawn)
	{
		...
		Tile = WorldData.GetTileCoordinatesFromPosition(Spawn.Location); // PI Added
		if(TilesFromSpawn > MinimumTilesFromLocation 
			...
			&& class'X2TargetingMethod_EvacZone'.static.ValidateEvacArea(Tile, false)) // PI Added: And only if the evac zone as a whole is valid
		{
