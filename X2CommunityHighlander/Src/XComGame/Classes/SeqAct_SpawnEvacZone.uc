...
private function Vector GetSpawnLocation()
{
	...
	local TTile Tile; // PI Added - Issue #264
	...
	foreach `BATTLE.AllActors(class'XComGroupSpawn', Spawn)
	{
		...
		Tile = WorldData.GetTileCoordinatesFromPosition(Spawn.Location); // Issue #264 - PI Added
		if(TilesFromSpawn > MinimumTilesFromLocation 
			...
			&& class'X2TargetingMethod_EvacZone'.static.ValidateEvacArea(Tile, false)) // Issue #264 - PI Added: And only if the evac zone as a whole is valid
		{
