...
//----------------------------------------------------------------------------
// robojumper: issue #1: screen extensions: add default behavior for screen extensions (dedicated popup screen)
...
// return the extension component of this screen. if bCreateLazy is false, it won't be created if not present
simulated function UIScreenComponent_ExtensionContainer GetExtensionContainer(optional bool bLazyCreate = true)
{
	local UIScreenComponent_ExtensionContainer Container;

	foreach ComponentList(class'UIScreenComponent_ExtensionContainer', Container)
	{
		return Container;
	}
	if (bLazyCreate)
	{
		Container = new (self) class'UIScreenComponent_ExtensionContainer';
		AttachComponent(Container);
	}
	return Container;
}

// used by UIScreenStack to determine whether to allow showing the extension screen
// override in case you don't want it to be shown
simulated function bool CanShowExtensionScreen()
{
	return GetExtensionContainer(false) != none && GetExtensionContainer().Extensions.Length > 0;
}

// whether to show a visual indicator that the extension screen is available
// override in case you don't want an indicator or on a different location
// TODO: where, how?
simulated function bool ShouldShowExtensionHint(out int HintAnchor, out int HintX, out int HintY)
{
	HintAnchor = class'UIUtilities'.const.ANCHOR_TOP_RIGHT;
	HintX = 0;
	HintY = 0;
	return CanShowExtensionScreen();
}

// show the extension screen. Override for different screen types
simulated function bool ShowExtensionScreen(optional class<UIScreen_ExtensionScreen> ScreenClass = class'UIScreen_DefaultExtensionScreen')
{
	local UIScreen_ExtensionScreen ExScreen;
	ExScreen = Spawn(ScreenClass, PC.Pres);
	Movie.Stack.Push(ExScreen);
	ExScreen.RealizeExtensions();
	return true;
}

// override for custom behavior (ex. update list items)
simulated function RealizeExtensions()
{
	local UIScreen ExtensionScreen;
	ExtensionScreen = GetActiveExtensionScreen();
	if (ExtensionScreen != none)
	{
		ExtensionScreen.RealizeExtensions();
	}
}

// returns the currently active extension screen FOR THIS SCREEN
// or none, if there isn't one
simulated function UIScreen_ExtensionScreen GetActiveExtensionScreen()
{
	local UIScreenStack Stack;
	local int i;

	Stack = `SCREENSTACK;
	// go down the stack, find us, go up to find the extension screen
	for (i = 0; i < Stack.Screens.Length; i++)
	{
		if (Stack.Screens[i] == self)
		{
			for (i = i-1; i >= 0; i--)
			{
				if (UIScreen_ExtensionScreen(Stack.Screens[i]) != none)
				{
					// the screen is an extension screen, return it
					return UIScreen_ExtensionScreen(Stack.Screens[i]);
				}
				else if (Stack.Screens[i] != none && UIMouseGuard(Stack.Screens[i]) == none)
				{
					// it's not an extension screen, not a mouse guard, a different screen. ABORT
					return none;
				}
			}
			break;
