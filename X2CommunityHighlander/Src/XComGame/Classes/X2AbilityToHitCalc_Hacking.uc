...
static function int GetHackDefenseForTarget(XComGameState_BaseObject TargetState)
{
	...
	//`LOG ("GetHackDefenseForTarget Triggered");
	...
	// Pavonis Fix for bug when hack defense drops below zero
	if (UnitState != None)
	{
		//`LOG ("Hack Defense:" @ UnitState.GetCurrentStat(eStat_HackDefense));
		if (UnitState.GetCurrentStat(eStat_HackDefense) > 0)
		{
