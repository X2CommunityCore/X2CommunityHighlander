...
// extension panel activation is handled by the screenstack to ensure maximum compatibility
...
//	local XComHQPresentationLayer HQPres;
	...
	if (MaybeShowExtensionPanel(iInput, ActionMask))
	{
		return true;
	}
...
// robojumper: issue #1: screen extensions start
simulated function bool MaybeShowExtensionPanel(int cmd, int arg)
{
	...
	if (class'Helpers_LW'.default.arrExtensionScreenKeys.Find(cmd) != INDEX_NONE)
	{
		return GetCurrentScreen().CanShowExtensionScreen() && GetCurrentScreen().ShowExtensionScreen();
	}
	return false;
}
// robojumper: issue #1: screen extensions end
